...
function parse(from) {
    ...
    schema_map_types: for (const { value } of lu) {
        if (value instanceof parser_internals_1.Message || value instanceof parser_internals_1.Extends)
            for (const field of (value instanceof parser_internals_1.Message ? value.fields : value.msg.fields))
                if (field.map) {
                    if (parser_internals_1.MAP_TYPES.includes(field.map.from) ||
                        find_lookup(schema, field.map.from, "enum"))
                        continue;
                    throw new SyntaxError(`Fields of type map cannot use ${field.map.from} as a key value, please use an enum, integer, or string type (${parser_internals_1.MAP_TYPES.join(', ')} + enum)`);
                }
    }
