...
function* find_lookups(schema, fis = 'any') {
...
exports.find_lookups = find_lookups;
...
function* ancestors_of(child) {
    let ancestor = parents_tree.get(child);
    while (ancestor) {
        if (ancestor instanceof schema_1.Schema)
            break;
        yield ancestor;
        ancestor = parents_tree.get(child);
    }
}
exports.ancestors_of = ancestors_of;
function parse(from) {
    ...
    map_types: for (const { value } of lu) {
        if (value instanceof parser_internals_1.Message || value instanceof parser_internals_1.Extends)
            for (const field of value.fields)
                if (field.map) {
                    if (parser_internals_1.MAP_KEY_TYPES.has(field.map.from) ||
    ...
    for (const msg of find_lookups(schema, "message"))
        fields: for (const field of msg.fields) {
            if (field.packed && !parser_internals_1.PACKABLE_TYPES.has(field.type)) {
                // check for enum type
                {
            ...
            if (!parser_internals_1.PRIMITIVE_TYPES.has(field.type)) {
