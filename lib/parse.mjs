...
import { PACKABLE_TYPES, TokenCount, on_syntax_version, on_package_name, on_enum, on_message, on_option, on_import, on_extend, on_service, Message, Extends, MAP_TYPES } from "./parser-internals";
...
export function parse(from) {
    ...
    schema_map_types: for (const { value } of lu) {
        if (value instanceof Message || value instanceof Extends)
            for (const field of (value instanceof Message ? value.fields : value.msg.fields))
                if (field.map) {
