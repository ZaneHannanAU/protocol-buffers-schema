...
exports.MAP_KEY_TYPES = Object.freeze([
    // varint wire types
    'int32', 'int64', 'uint32', 'uint64', 'sint32', 'sint64', 'bool',
    // + ENUMS
    // 64-bit wire types
...
class Options {
    parse_bool(name) {
        if (opts_wm.has(this))
            switch (opts_wm.get(this).get(name)) {
                case "true": return true;
                case "false": return false;
            }
        return null;
    ...
    while (!n.done)
        switch (n.peek()) {
            ...
            default: {
                ...
                if (isNaN(value))
                    n.syntax_err(`Enum value with name ${name} must not be NaN`);
                if (value > 2147483647)
                    n.syntax_err(`Enum value must not be greater than 32 bit signed integer limit`);
                value |= 0;
                if (value < 0)
